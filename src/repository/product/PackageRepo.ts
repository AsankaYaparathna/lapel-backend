...
export class PackageRepo implements IPackageRepo {
  async create(model: any): Promise<void> {
    try {
      const result = await CstomProductPackages.findOne({ where: { title: model.title }, });
      if (result) {
        throw new Error(
          "Failed to create Custom Product Package! Custom Product Package with this name already exists!"
        );
      }
      ...
      const newCreatedModel = await CstomProductPackages.create({
        title: model.title,
